...
const getAlldataCookfinished = async (req, res) => {
  try {
    const userRole = req.user.role;

    if (userRole !== 'waiter') {
      return res.status(403).json({ message: 'Akses ditolak, hanya waiters yang dapat mengakses ini' });
    }
...
module.exports = { getAlldataCookfinished };
