...
const PesananSelesaiDimasakPages = () => {
  ...
  const handleConfirmDelivery = (orderId) => {
    Swal.fire({
      title: `Apakah pesanan sudah sampai ke pelanggan?`,
      text: 'Harap pastikan pesanan telah sampai ke pelanggan sebelum menekan tombol ini! Kesalahan dapat berakibat fatal bagi layanan kami dan pelanggan.',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await axios.post('http://localhost:3001/api/users/Waiters/delivery', {orderId}, {
              headers: {
          ...
          if (response) {
            Swal.fire({
              title: 'Pengantaran Berhasil Dikonfirmasi',
              text: 'Status pesanan telah diperbarui selesai diantar ke pelanggan.',
              icon: 'success',
            ...
            Swal.fire({
              ...
              text: 'Konfirmasi pengantaran tidak berhasil. Silakan coba lagi.',
              icon: 'error',
              confirmButtonText: 'Oke',
            });
          }
        ...
        Swal.fire({
          ...
          text: 'Pengantaran tidak dikonfirmasi. Pastikan untuk melakukan konfirmasi jika pesanan sudah sampai.',
          icon: 'info',
          confirmButtonText: 'Oke',
        });
      }
  ...
  useEffect(() => {
    dispatch(setActiveItem("Selesai Dimasak"));
  ...
  useEffect(() => {
    getAllDataOrderDoneCooking();
  }, []);

  const getAllDataOrderDoneCooking = async () => {
    try {
      const token = localStorage.getItem('token');
        ...
        <div className="p-6">
          <div className='mb-4'>
            ...
            <h1 className="text-red-600 text-sm ml-2 p-1 rounded-md font-Poppins text-right pl-10 md:pl-32">
              Harap antar pesanan segera! Pastikan pesanan sampai dengan aman sebelum menekan tombol konfirmasi antar!
            </h1>
          </div>
...
export default PesananSelesaiDimasakPages;
